...
export default function Login() {
  ...
  return (
    <div className="flex items-center  max-sm:flex-col bg-gradient-to-b from-blue-500 via-blue-700 to-blue-900 h-screen overflow-y-auto">
        ...
        <div className={`sm:w-1/2 w-full h-full duration-100 ease-in-out  shadow-xl relative  duration-100 ease-in-out bg-white ${isUp ? "rounded-t-0" : "rounded-t-3xl"}   m-auto flex justify-center text-neutral-500`}>
          <div className="flex   flex-col gap-2 justify-center">
        <ChevronUpIcon onClick={()=>setIsUp(!isUp)} className={`size-8 bg-neutral-100 rounded-full mx-auto absolute top-4 hover:scale-105 duration-75 cursor-pointer left-0 right-0 ${isUp ? " rotate-180" : "rotate-0"} duration-150 ease-in-out text-neutral-500`} />
